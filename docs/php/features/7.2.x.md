...
### 2.2. [__autoload()](http://php.net/manual/zh/function.autoload.php) 方法
...
### 2.3. [create_function()](http://php.net/manual/zh/function.create-function.php) 函数
...
### 2.5. [parse_str()](http://php.net/manual/zh/function.parse-str.php) 不加第二个参数
...
### 2.6. [each()](http://php.net/manual/zh/function.each.php) 函数

使用此函数遍历时，比普通的 `foreach` 更慢， 并且给新语法的变化带来实现问题。因此它被废弃了。

### 2.7. [assert()](http://php.net/manual/zh/function.assert.php) 一个字符串参数
